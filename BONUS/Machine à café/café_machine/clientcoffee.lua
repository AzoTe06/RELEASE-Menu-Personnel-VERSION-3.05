...
function Main()
    ...
    Menu.addButton("Barre Choco (80$)", "Choco", nil)
end
...
function Choco()
    TriggerServerEvent("Choco_Server")
  Menu.hidden = false
end

RegisterNetEvent("Choco")
AddEventHandler("Choco",  function()
		...
		for k,v in ipairs(twentyfourseven_shops) do
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 20.0)then
				if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 2.0)then
					DisplayHelpText("Appuyer sur ~INPUT_VEH_EXIT~ pour ~g~acheter un à la machine.")
